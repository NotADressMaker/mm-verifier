...
export interface MethodManifest {
  extractor: string;
  classifier: string;
  evidenceRetriever: string;
  relationClassifier: string;
  metrics: string;
}
...
export interface AtomicClaim {
  ...
  claimType: ClaimType;
  verifiability: 'directly_verifiable' | 'partially_verifiable' | 'interpretation_dependent' | 'not_empirically_verifiable';
}
...
export interface ClaimEvidenceRelation {
  ...
  scopeMatch?: { semantic: boolean; temporal: boolean; jurisdictional: boolean };
}
...
export interface EducationalFeedback {
  claimId?: string;
  category: 'claim_precision' | 'evidence_quality' | 'source_independence' | 'reasoning' | 'counterargument' | 'uncertainty' | 'citation' | 'revision';
...
export interface EducationVerdictPolicy {
  id: string;
  version: string;
  evaluate(metrics: EducationalAssessmentMetrics, claim: AtomicClaim, evidence: ClaimEvidenceRelation[]): ClaimAssessmentStatus;
}

export interface EducationVerificationReport {
  id: string;
  responseId: string;
  verificationFrame: VerificationFrame;
  summary: {
    overallStatus: ClaimAssessmentStatus;
    totalClaims: number;
    supportedClaims: number;
    contradictedClaims: number;
    unresolvedClaims: number;
    interpretationDependentClaims: number;
  };
  claims: Array<{
    claim: AtomicClaim;
    assessment: { status: ClaimAssessmentStatus; explanation: string; evidentialStrength?: number; limitations: string[] };
    evidenceRelations: ClaimEvidenceRelation[];
    feedback: EducationalFeedback[];
  }>;
  metrics: EducationalAssessmentMetrics;
  reflectionPrompts: string[];
  integrity: { claimSetHash?: string; frameHash?: string; evidenceRoot?: string; receiptHash?: string };
}
...
export interface EducationVerifyRequest {
  ...
  jurisdiction?: string;
  interpretation?: string;
  providedSources?: Array<{ title?: string; url?: string; citation?: string; independenceCluster?: string }>;
}

export interface EducationVerifyResponse {
  reportId: string;
  overallStatus: ClaimAssessmentStatus;
  claims: EducationVerificationReport['claims'];
  metrics: EducationalAssessmentMetrics;
  feedback: EducationalFeedback[];
  reflectionPrompts: string[];
  receipt: Record<string, unknown>;
  report: EducationVerificationReport;
}
