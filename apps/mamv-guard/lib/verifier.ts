...
import { resolvePolicy, isAccepted } from './policies';
...
export async function verifyWithMamv(input: VerifyRequestInput) {
  ...
  try {
    ...
    const verdict = Boolean(receipt.verdict);
    const decision = isAccepted(receipt.score_bps, verdict, policy);
    const updated = await prisma.verification.update({ where: { id: record.id }, data: { taskId: task.task_id, status: decision.accepted ? 'VERIFIED' : 'REJECTED', verdict, worthy: decision.worthy, scoreBps: receipt.score_bps, receiptJson: JSON.stringify(receipt), evidenceJson: JSON.stringify(receipt.evidence), receiptHash: receipt.receipt_hash, receiptUrl: receiptUrl(record.id), badgeUrl: badgeUrl(record.id), chainId: receipt.chain_context?.chain_id, contractAddress: receipt.chain_context?.contract_address, txHash: onchain?.chain_data?.tx_hash, blockNumber: onchain?.chain_data?.block_number } });
    const response = toPublicResult(updated);
    if (policy.webhooks.length) await deliverWebhooks(record.id, policy.webhooks, response);
    return response;
  } catch (error) {
    const updated = await prisma.verification.update({ where: { id: record.id }, data: { status: 'FAILED', error: error instanceof Error ? error.message : String(error) } });
    return toPublicResult(updated);
  }
}

export function toPublicResult(v: any) {
  return { id: v.id, taskId: v.taskId, status: v.status, verdict: v.verdict, worthy: v.worthy, scoreBps: v.scoreBps, thresholdBps: v.thresholdBps, receiptHash: v.receiptHash, receiptUrl: v.receiptUrl, badgeUrl: v.badgeUrl, anchor: { requested: v.anchorRequested, chainId: v.chainId, contractAddress: v.contractAddress, txHash: v.txHash, blockNumber: v.blockNumber }, error: v.error, createdAt: v.createdAt, updatedAt: v.updatedAt };
}
