...
import { EducationClaim, EducationResult, EducationVerifyRequest, EpistemicLiteracyAssessment, VerificationDimensions } from '../../../shared/education/types';
...
function assessEpistemicLiteracy(content: string, claims: EducationClaim[], citationReview: ReturnType<typeof citations>): EpistemicLiteracyAssessment {
  const indicators: EpistemicLiteracyAssessment['indicators'] = [
    { id: 'claim', label: 'Makes a checkable claim', status: claims.some(claim => claim.classification !== 'not_evaluated') ? 'observed' : 'not_observed', detail: 'A checkable statement lets a learner distinguish an assertion from an unsupported conclusion.' },
    { id: 'evidence', label: 'Connects a claim to evidence', status: citationReview.status === 'format_checked' ? 'observed' : 'not_observed', detail: citationReview.status === 'format_checked' ? 'A citation-like reference or supplied source was detected; this demo checks format, not source quality.' : 'Add a course-approved source or describe the observation, data, or text that supports the claim.' },
    { id: 'qualification', label: 'States uncertainty or conditions', status: /\b(may|might|could|likely|unlikely|depends|if|unless|however|limit(?:ation)?s?)\b/i.test(content) ? 'observed' : 'not_observed', detail: 'Qualifying a claim makes its scope and uncertainty available for review.' },
    { id: 'alternative', label: 'Considers an alternative explanation', status: /\b(alternative|another explanation|on the other hand|whereas|although|however|counter(?:claim|argument)|but)\b/i.test(content) ? 'observed' : 'not_observed', detail: 'Considering alternatives helps separate a conclusion from the path used to reach it.' },
  ];
  const observed = indicators.filter(indicator => indicator.status === 'observed').length;
  const next_steps = indicators.filter(indicator => indicator.status === 'not_observed').map(indicator => ({ claim: 'State one specific claim that can be checked.', evidence: 'Link the claim to a course-approved source or observation.', qualification: 'Name a condition, uncertainty, or limitation on the claim.', alternative: 'Compare the claim with one plausible alternative explanation.' })[indicator.id]);
  return { score: Number((observed / indicators.length).toFixed(2)), summary: `Observed ${observed} of ${indicators.length} visible reasoning practices. This is a process prompt, not a grade or a measure of hidden reasoning.`, indicators, next_steps };
}
export function verifyEducation(request: EducationVerifyRequest) {
  ...
  const epistemic_literacy = assessEpistemicLiteracy(request.content, claims, citation_review);
