...
import { EducationClaim, EducationResult, EducationVerifyRequest } from '../../../shared/education/types';
...
export function verifyEducation(request: EducationVerifyRequest) {
  const claims = sentenceSplit(request.content).slice(0, 12).map((text, index): EducationClaim => {
    ...
    return { id: `claim-${String(index + 1).padStart(2, '0')}-${hashUtf8(text).slice(2, 8)}`, text, classification, confidence, explanation: classification === 'supported' ? 'Demo cross-checkers did not identify a contradiction.' : classification === 'not_evaluated' ? 'This statement is not a factual claim that this MVP can meaningfully check.' : 'This claim needs human review; this MVP does not retrieve authoritative sources.', evidence: [], warnings: classification === 'supported' ? [] : ['Review this claim against course-approved sources.'] };
  });
  ...
  const result: EducationResult = { support_level: supportLevelForScore(score), confidence_score: Number(score.toFixed(2)), summary: `${claims.filter(c => c.classification === 'supported').length} of ${claims.length} extracted claims received stronger cross-checking signals. This is evidence, not a guarantee of truth or academic acceptability.`, claims, model_agreement: { agreement_score: disagreement ? .58 : .9, votes: [{ provider: 'demo', model: 'mamv-education-crosscheck-a', vote: disagreement ? 'mixed' : 'support', score }, { provider: 'demo', model: 'mamv-education-crosscheck-b', vote: disagreement ? 'review' : 'support', score: disagreement ? .55 : .9 }], outliers: disagreement ? ['mamv-education-crosscheck-b'] : [] }, citation_review, review_questions: claims.filter(c => c.classification !== 'supported').map(c => `What course-approved source supports or challenges: “${c.text}”`).slice(0, 3), warnings: [...citation_review.warnings, ...(request.mode === 'teacher_content' && /answer key|correct answer/i.test(request.content) ? ['Check answer-key choices against each question before classroom use.'] : [])], limitations: ['Demo mode is deterministic and visibly simulated; it is not live model or source verification.', 'No authoritative source content was retrieved or reviewed.', 'MAMV provides evidence and confidence signals, not guarantees of truth, originality, fairness, or academic acceptability.'], demo_mode: true };
  const task_id = `edu_${hashUtf8(`${request.mode}:${request.content}`).slice(2, 18)}`;
  const receipt = { receipt_version: '1.0.0', receipt_id: hashCanonical({ task_id, score, input: hashUtf8(request.question || '') }), generated_at: new Date().toISOString(), task_id, input_hash: hashUtf8(request.question || ''), output_hash: hashUtf8(request.content), verification_status: result.support_level, confidence_score: score, education: { mode: request.mode, subject: request.subject, education_level: request.education_level, content_type: request.content_type, raw_content_persisted: false }, claim_summary: { total: claims.length, supported: claims.filter(c => c.classification === 'supported').length }, votes: result.model_agreement.votes, outliers: result.model_agreement.outliers, quorum_status: { met: result.model_agreement.agreement_score >= .67, participant_count: 2 }, warnings: result.warnings, limitations: result.limitations, signature_status: 'hash_only_not_signed', onchain_anchor: { anchor_status: 'not_anchored', disabled_for_education: true }, receipt_hash: hashCanonical({ task_id, result }) };
